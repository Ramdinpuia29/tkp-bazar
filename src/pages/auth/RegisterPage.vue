...
const registerWithCredentials = async () => {
  ...
  loading.value = true;
  try {
    await authStore.registerWithCredentials(registerForm.email, registerForm.password);
    router.push({ name: 'feed', replace: true });
...
<template>
  <q-page class="flex justify-center items-center full-width">
    <div>
      <q-card class="shadow-5 q-ma-md" style="border-radius: 1em;">
        <q-card-section>
          <span class="text-h4 text-primary q-mx-md">Register</span>
          <q-separator class="q-ma-md" color="primary" />
          <form @submit.prevent="registerWithCredentials">
            <q-input class="q-my-md q-px-md" v-model="registerForm.email" label="Email">
              <template v-slot:prepend>
                <q-icon name="fa-solid fa-at" color="primary" />
              </template>
            </q-input>
            <q-input class="q-my-md q-px-md" v-model="registerForm.password" label="Password"
            ...
            <q-card-actions align="center">
              <q-btn class="q-pa-md bg-primary text-white full-width" type="submit" size="md"
                style="border-radius: 0.5em;">Register</q-btn>
          ...
          <div class="flex flex-center q-mt-md">
            <span class="text-center">Already have account?<a href="#/auth/login"
                class="cursor-pointer text-primary text-bold" style="text-decoration: none;"> Log in </a></span>
          </div>
        </q-card-section>
      </q-card>
    </div>
